...
export default function MenuManagement() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setMaster(mr.data.data); setWeeks(wr.data.data);
  ...
  const resetWeek = async () => {
    ...
    try { const r = await resetWeekPlan({ week_start: weekStart }); toast.success(r.data.message); await loadPlan(weekStart); const wr = await getAvailableWeeks(); setWeeks(wr.data.data); }
        ...
        {tab === 'weekly' && (
          <>
            <div className="card" style={{ marginBottom:'1.25rem', padding:'1rem 1.25rem' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'1rem', flexWrap:'wrap' }}>
                <div className="form-group" style={{ marginBottom:0 }}>
                  ...
                  <input type="date" className="form-control" value={weekStart} onChange={e => { setWeekStart(e.target.value); loadPlan(e.target.value); }} style={{ minWidth:160 }} />
                </div>
            ...
            <div style={{ display:'flex', gap:'.4rem', flexWrap:'wrap', marginBottom:'1rem' }}>
              {DAYS.map(d => {
                const f = MEALS.filter(m => slotCount(d,m)>0).length;
