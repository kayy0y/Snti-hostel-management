...
export const approveRegistration = (id, action) =>
  API.put(`/registrations/${id}/approve`, { action });
...
export const getAllMenuSelections = ws =>
  API.get(`/menus/all-selections${ws ? `?week_start=${ws}` : ''}`);

// Weekly plan
export const getWeeklyPlan = ws =>
  API.get(`/weekly-plan${ws ? `?week_start=${ws}` : ''}`);

export const addItemToPlan = d =>
  API.post('/weekly-plan/add-item', d);

export const removeItemFromPlan = pid =>
  API.delete(`/weekly-plan/remove-item/${pid}`);

export const resetWeekPlan = d =>
  API.delete('/weekly-plan/reset', { data: d });

export const getAvailableWeeks = () =>
  API.get('/weekly-plan/available-weeks');

// Feedback
export const submitFeedback = d =>
  API.post('/feedback', d);
...
export const addStudent = d =>
  API.post('/admin/students', d);

export const deleteStudent = id =>
  API.delete(`/admin/students/${id}`);

export const deleteStudentNow = (id, archive = false) =>
  API.delete(`/admin/students/${id}/now`, {
    data: { archive }
  });

export const resetBatch = (archive = false) =>
  API.post('/admin/reset-batch', { archive });
...
export const getAnalytics = ws =>
  API.get(`/admin/analytics${ws ? `?week_start=${ws}` : ''}`);
...
export const createAdmin = d =>
  API.post('/admin/create-admin', d);
...
export const runArchive = (month, year) =>
  API.post('/archive/run', { month, year });
